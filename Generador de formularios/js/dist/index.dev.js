...
*/
var elementosStandardFormulario = new Map();
...
var cantidadComponentes = 0; //Este metodo muestra un menu de propiedades 
...
function mostrarMenuPropiedadesElementos(tipoElemento, crearModificar) {
  ...
  if (crearModificar == "crear") {
    ...
    $("#buttonAnyadirModificar").off("click").click(function (event) {
      var p = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [tipoElemento, inputStandard];
      addElement(p);
    });
  } else {}
} //Recibe un array
...
function addElement(argumentos) {
  var tipo = [argumentos[0], null]; //Se lee el input del menu propiedades, el texto introducido se usara como id del componente y como texto para su label

  var $inputMenuPropiedades = $("#inputMenuPropiedades"); //Aqui se declara un input simple y que ademas el tipo de este se defina en el id, si no se define en el id posteriormente se
  //sobrescribira con un nuevo input

  var $input = $('<' + tipo[0] + ' class="inputElemento form-control" >'); //En la siguiente variable se guarda el div donde ira el o los inputs

  var $componente = $('<div class="contenedor" id="' + cantidadComponentes + '">').addClass("elementosFormulario"); //En el siguiente let se crea el label del componente
    ...
    if (argumentos[1][0] == "complejo") {
      switch (argumentos[0]) {
        case ("checkbox", "radio"):
          ...
          var $label = $('<label class="form-check-label" for="' + $("#input1").val() + '1" >').html($('#input1').val());
  ...
  console.log(argumentos); //$componente=$('<div>').addClass("elementosFormulario");
...
function ocultarMenuPropiedas() {
  ...
  $('#menuElemento').css("display", "inline"); //Se hace displey block a los li que estan dentro del div menuElemento para que se vean uno debajo del otro
...
function mostrarInputsMenuComplejo(cantidad) {
  ...
  if (cantidad > 1) {
    for (var index = 1; index < cantidad; index++) {
      var inp = $('#divInputReferencia').clone();
